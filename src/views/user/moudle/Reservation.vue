<template>
    ...
    <div>
        ...
        <h1 style="text-align: center">预约信息管理</h1>
        ...
        <el-table
            ...
            <el-table-column
                ...
                <template slot-scope="scope">
                    ...
                    <el-button @click="delete_a(scope.row)" type="info" size="small">删除预约</el-button>
                </template>
            </el-table-column>
        </el-table>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            delete_a(row) {
                const _this = this
                _this.$api.makeApi.delect(row.mid).then(res =>{
                    if (res == 1){
                        alert("删除成功")
                    }else {
                        alert("删除失败，详情到日志查看")
            ...
            pagePrev(){
                let _this = this
                _this.$api.makeApi.listByAll(_this.pageNum - 1, 8).then(res => {
            ...
            page(number){
                let _this = this
                _this.$api.makeApi.listByAll(number, 8).then(res => {
                    _this.tableData = res.list
                    _this.total = res.total
                    _this.pageNum = res.pageNum
                })
        ...
        created() {
            const _this = this
            _this.$api.makeApi.listByAll(1, 8).then(res => {
                _this.tableData = res.list
                _this.total = res.total
                _this.pageNum = res.pageNum
            })
