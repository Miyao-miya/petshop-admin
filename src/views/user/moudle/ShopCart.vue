<template>
    ...
    <div>
        ...
        <h1 style="text-align: center">购物车信息管理</h1>
        ...
        <el-table
            ...
            <el-table-column
                ...
                <template slot-scope="scopeImg">
                    <el-image
                            ...
                            :src="scopeImg.row.fkGId.gimg"
                            :preview-src-list="scopeImg.row.fkGId.gimgs">
                    </el-image>
                </template>
            </el-table-column>
            <el-table-column
            ...
            <el-table-column
                ...
                <template slot-scope="scope">
                    ...
                    <el-button @click="delete_a(scope.row)" type="info" size="small">移出购物车</el-button>
                </template>
            </el-table-column>
        </el-table>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            delete_a(row) {
                const _this = this
                _this.$api.shopCartApi.delect(row.mid).then(res =>{
                    if (res == 1){
                        alert("删除成功")
                    }else {
                        alert("删除失败，详情到日志查看")
            ...
            pagePrev(){
                let _this = this
                _this.$api.shopCartApi.shopCartPage(_this.pageNum - 1, 8).then(res => {
            ...
            page(number){
                let _this = this
                _this.$api.shopCartApi.shopCartPage(number, 8).then(res => {
                    _this.tableData = res.list
                    _this.total = res.total
                    _this.pageNum = res.pageNum
                })
        ...
        created() {
            const _this = this
            _this.$api.shopCartApi.shopCartPage(1, 8).then(res => {
                _this.tableData = res.list
                _this.total = res.total
                _this.pageNum = res.pageNum
            })
