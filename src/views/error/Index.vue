<template>
    ...
    <div>
        ...
        <h1 style="text-align: center">错误日志</h1>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            pagePrev(){
                let _this = this
                _this.$api.errorApi.listByAll(_this.pageNum - 1, 8).then(res => {
            ...
            page(number){
                let _this = this
                _this.$api.errorApi.listByAll(number, 8).then(res => {
                    _this.tableData = res.list
                    _this.total = res.total
                    _this.pageNum = res.pageNum
                })
        ...
        created() {
            const _this = this
            _this.$api.errorApi.listByAll(1, 8).then(res => {
                _this.tableData = res.list
                _this.total = res.total
                _this.pageNum = res.pageNum
            })
